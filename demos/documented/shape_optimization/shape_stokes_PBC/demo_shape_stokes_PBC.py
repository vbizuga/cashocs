...
# a no-slip boundary condition on the obstacle boundary $\Gamma^\text{obs}$,
# and a do-nothing condition on the outlet $\Gamma^\text{out}$. Additionaly,
# we have a periodic boundary condition the wall boundary $\Gamma^\text{wall}$.
#
...
# Note that with periodic boundary conditions, the barycenter constraint is in this
# case not obligatory, as the center is automaticaly aligned in the center of the mesh.
...
# ## Implementation
#
# The complete python code can be found in the file {download}`demo_shape_stokes_PBC.py
# </../../demos/documented/shape_optimization/shape_stokes_PBC/demo_shape_stokes_PBC.py>`,
# and the corresponding config can be found in {download}`config.ini
# </../../demos/documented/shape_optimization/shape_stokes_PBC/config.ini>`.
#
# ### Initialization
#
...
mesh, subdomains, boundaries, dx, ds, dS = cashocs.import_mesh("./mesh/mesh.msh")

# ::::{note}
# As defined in `./mesh/mesh.geo`, we have the following boundaries
#
# - The inlet boundary $\Gamma^\text{in}$, which has index {python}`1`
# - The wall boundaries $\Gamma^\text{wall}$, which have index {python}`2 and 3` (bottom and top)
# - The outlet boundary $\Gamma^\text{out}$, which has index {python}`4`
# - The boundary of the obstacle $\Gamma^\text{obs}$, which has index {python}`5`
#
# To include the fact that only the obstacle boundary, with index {python}`5`, is
...
u_in = Expression(("-1.0/4.0*(x[1] - 3.0)*(x[1] + 3.0)", "0.0"), degree=2)
bc_in = DirichletBC(V.sub(0), u_in, boundaries, 1)

# The obstacle boundary gets a no-slip boundary condition with the line

bc_no_slip = cashocs.create_dirichlet_bcs(
    V.sub(0), Constant((0, 0)), boundaries, [5]
)

# The periodic boundary conditions on the walls are set by the line

bc_periodic = cashocs.create_periodic_bcs(
    V, boundaries, [3, 2], periodicboundary
)

# Finally, all Dirichlet boundary conditions and the periodic boundary conditions 
# are gathered into the list {python}`bcs`

bcs = [bc_in] + bc_no_slip + bc_periodic
